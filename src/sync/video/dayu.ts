...
export async function VideoDayu(data: SyncData): Promise<boolean> {
  ...
  try {
    ...
    const DayuVideoUploader = class DayuVideoUploader {
      ...
      public async uploadHorizontalCover(coverData: any): Promise<boolean> {
        ...
        try {
          ...
          const success = await this.performCoverUpload(uploadArea as HTMLElement, file, 'horizontal');
      ...
      public async uploadVerticalCover(coverData: any): Promise<boolean> {
        ...
        try {
          ...
          const success = await this.performCoverUpload(uploadArea as HTMLElement, file, 'vertical');
        ...
        try {
            ...
            for (const label of Array.from(tagLabels)) {
              if (label.textContent?.includes('标签')) {
                ...
                if (fieldContent) {
                  ...
                  const input = fieldContent.querySelector('input, textarea') as HTMLInputElement | HTMLTextAreaElement | null;
                  if (input && (input as HTMLElement).offsetParent !== null && (input.tagName === 'INPUT' || input.tagName === 'TEXTAREA')) {
          ...
          if (!tagInput) {
            ...
            for (const elem of Array.from(vueElements)) {
              const parent = elem.closest('.article-write_video-tags, [class*="tag"]');
              if (parent && (elem as HTMLElement).offsetParent !== null && (elem.tagName === 'INPUT' || elem.tagName === 'TEXTAREA')) {
          ...
          if (!tagInput && tagContainer) {
            ...
            for (const child of Array.from(allChildren)) {
              ...
              if (tagName === 'INPUT' || tagName === 'TEXTAREA') {
                ...
                if (text.includes('tag') || text.includes('label') || text.includes('input') || tagName === 'INPUT' || tagName === 'TEXTAREA') {
          ...
          if (!tagInput) {
            ...
            if (tagContainer) {
              ...
              const dynamicInput = tagContainer.querySelector('input, textarea') as HTMLInputElement | HTMLTextAreaElement | null;
              if (dynamicInput && (dynamicInput as HTMLElement).offsetParent !== null && (dynamicInput.tagName === 'INPUT' || dynamicInput.tagName === 'TEXTAREA')) {
            ...
            if (!tagInput) {
              ...
              if (tagLabel) {
                ...
                const dynamicInput2 = document.querySelector('.article-write_video-tags input, .article-write_video-tags textarea') as HTMLInputElement | HTMLTextAreaElement | null;
                if (dynamicInput2 && (dynamicInput2 as HTMLElement).offsetParent !== null && (dynamicInput2.tagName === 'INPUT' || dynamicInput2.tagName === 'TEXTAREA')) {
      ...
      private async handleImageCropDialog(): Promise<void> {
        try {
          ...
          if (foundDialog) {
            ...
            const saveResult = await this.executeConfirmStrategy();
            if (saveResult) {
              console.log('✅ 图片裁剪弹窗保存成功');
              return;
            }
          } else {
    ...
    if (cover) {
      ...
      const horizontalCoverSuccess = await uploader.uploadHorizontalCover(cover);
      if (!horizontalCoverSuccess) {
        console.log('⚠️ 横版封面上传失败，继续...');
      }
    } else {
    ...
    if (verticalCoverData) {
      ...
      const verticalCoverSuccess = await uploader.uploadVerticalCover(verticalCoverData);
      if (!verticalCoverSuccess) {
        console.log('⚠️ 竖版封面上传失败，继续...');
      }
    } else {
    ...
    if (tags && tags.length > 0) {
      ...
      const tagSuccess = await uploader.uploadVideoTags(tags);
      if (!tagSuccess) {
        console.log('⚠️ 视频标签处理失败，继续...');
      }
    } else {
    ...
    const sourceSuccess = await uploader.selectVideoSource();
    if (!sourceSuccess) {
      console.log('⚠️ 信息来源设置失败，继续...');
    } else {
      console.log('✅ 信息来源设置成功');
    }
