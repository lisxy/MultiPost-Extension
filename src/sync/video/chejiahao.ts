        ...
        try {
          ...
          if (browserElement) {
            ...
            if (uploadLink) {
              ...
              // 检查a标签的内容变化来判断上传状态
              if (linkText.includes('上传中') || linkText.includes('已上传') ||
                  linkText.includes('上传速度') || linkText.includes('剩余时间')) {
          ...
          if (browserElementForUpload) {
            ...
            if (uploadLink) {
              ...
              if (linkText.includes('上传中') || linkText.includes('0.00%') ||
                  linkText.includes('已上传') || linkText.includes('上传速度')) {
            ...
            existingFileInputs.forEach((existingInput, index) => {
              ...
              if (existingInput.accept && existingInput.accept.includes('video')) {
            ...
            if (finalLink) {
              ...
              if (finalText.includes('上传中') || finalText.includes('0.00%') ||
                  finalText.includes('已上传') || finalText.includes('上传速度')) {
          ...
          if (!AHVP) {
            ...
            window.XMLHttpRequest = function() {
              ...
              xhr.open = function(method: string, url: string | URL, ...args: any[]) {
                if (url && url.toString().includes('upload')) {
            ...
            for (const button of uploadButtons) {
              ...
              if (text.includes('上传') || text.includes('发布') || text.includes('提交')) {
                ...
                if (text.includes('上传中') || text.includes('已上传')) {
                  ...
                  console.log('❌ 检测到上传失败状态');
                  return false;
                } else if (text.includes('上传完成') || text.includes('100%')) {
                  console.log('🎉 检测到上传完成状态');
                  return true;
                }
              }
          ...
          const config = {
            ...
            update: function(callback: (config: any) => void, config: any) {
              // 更新认证参数
              const newConfig = {
                ...config,
                _timestamp: Math.floor(Date.now() / 1000),
          ...
          browser.on(AHVP.UPLOADER_EVENT.PROGRESS, (item: any, progress: number) => {
            console.log('📊 上传进度:', Math.round(progress * 100) + '%');
          });

          browser.on(AHVP.UPLOADER_EVENT.SUCCESS, (item: any, response: any) => {
            console.log('✅ 上传成功:', response);
          });

          browser.on(AHVP.UPLOADER_EVENT.ERROR, (item: any, error: any) => {
          ...
          return new Promise((resolve) => {
            ...
            // 监听上传完成事件
            browser.on(AHVP.UPLOADER_EVENT.COMPLETED, (item: any, response: any) => {
